...
namespace Portal11.Select
{
    public partial class SelectVendor : System.Web.UI.Page
    {
        ...
        protected void AllVendorView_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            if (e.Row.RowType == DataControlRowType.DataRow)                // If == this is indeed a row of our GridView control
            {
                ...
                e.Row.Attributes["onclick"] = this.Page.ClientScript.GetPostBackClientHyperlink(this.AllVendorView, "Select$" + e.Row.RowIndex);
        ...
        protected void AllVendorView_SelectedIndexChanged(object sender, EventArgs e)
        {
        ...
        protected void AllVendorView_PageIndexChanging(object sender, GridViewPageEventArgs e)
        {
            if (e.NewPageIndex >= 0)                                        // If >= a value that we can handle
            {
                AllVendorView.PageIndex = e.NewPageIndex;                  // Propagate the desired page index
                LoadVendorView();                                          // Fill the grid
                AllVendorView.SelectedIndex = -1;                          // No row currently selected
            }
        }
        ...
        {
            Label vendID = (Label)AllVendorView.SelectedRow.Cells[0].FindControl("lblRowID"); // Find the label control that contains VendorID
            Label name = (Label)AllVendorView.SelectedRow.Cells[1].FindControl("lblName"); // Find the label control that contains Vendor Name
        ...
        void LoadVendorView()
        {
            using (Models.ApplicationDbContext context = new Models.ApplicationDbContext())
            {
                ...
                AllVendorView.DataSource = vends;                      // Give it to the GridView control
                AllVendorView.DataBind();                              // And display it

                NavigationActions.EnableGridViewNavButtons(AllVendorView); // Enable appropriate nav buttons based on page count
            }
        }
    }
}